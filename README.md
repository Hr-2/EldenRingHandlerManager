...
The repo uses a GitHub Actions workflow (`.github/workflows/build-release.yml`) that auto-versions every change — each push to `master` bumps the patch version, updates `CHANGELOG.md` from commit messages, tags it (e.g. `v1.1.0`), builds the `.exe`, and publishes a versioned release with changelog notes.
